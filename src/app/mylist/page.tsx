...
import { Favoritetype, MovieProps } from "@/types/main";
...
const Mylist = () => {
  ...
  const { account, pageLoading, setPageLoading } = useGlobalContext();

  useEffect(() => {
    const getMylistFavorite = async () => {
      try {
        const { data } = await getMyList(session?.user?.uid, account?._id);
        setFavorite(data);
      } catch (error) {
        toast({
          variant: "destructive",
          title: "Error",
          description: "There was a problem opening the account.",
        });
      } finally {
    ...
    <main className="flex min-h-screen flex-col">
      ...
      {favorite && favorite.length === 0 ? (
        ...
        <div className="space-y-4 px-4 mt-[12vh]">
          <h2 className="text-sm font-semibold text-[#e5e5e5] mt-6 md:text-2xl">My list</h2>

          {/* Grid layout */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {favorite.map((item: Favoritetype, index) => (
              <MovieItem
                key={index}
                moviesRun={{
